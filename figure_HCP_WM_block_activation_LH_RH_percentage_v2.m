...
load(fullfile(files(2).folder,files(2).name), ...
    'f0bk_faces_mean_signal','f0bk_faces_mean_signal_SubID','f0bk_body_mean_signal','f0bk_body_mean_signal_SubID', ...
...
[~,locb] = ismember(cov.SubID,cell2mat(cellfun(@str2num, f0bk_faces_mean_signal_SubID, 'UniformOutput', false)));
...
% load baselines
load(fullfile(datapath,'data/HCP_WM_block_BOLD_timeseries.mat'));
avg_sub_block = avg_sub_block(:,locb);
avg_f0bk_faces_baseline = mean([avg_sub_block.f0bk_faces]);
avg_f0bk_places_baseline = mean([avg_sub_block.f0bk_places]);
avg_f0bk_body_baseline = mean([avg_sub_block.f0bk_body]);
avg_f0bk_tools_baseline = mean([avg_sub_block.f0bk_tools]);
...
fig_savefolder = fullfile(savefolder,'fig','HCP_WM_activation_LH_RH_percentage/');
...
minVal = -0.5; maxVal = 1.4;

%% ======================== f0bk_faces ========================
faces_LH = mean(f0bk_faces(1:180,:),2) / avg_f0bk_faces_baseline * 100;
faces_RH = mean(f0bk_faces(181:360,:),2) / avg_f0bk_faces_baseline * 100;
...
set(gca,'YTick',[-0.5:0.5:1]);
xtickangle(90);
x_star=last90; y_star=repmat(-0.45,length(x_star),1);
...
%% ======================== f0bk_places ========================
places_LH = mean(f0bk_places(1:180,:),2) / avg_f0bk_places_baseline * 100;
places_RH = mean(f0bk_places(181:360,:),2) / avg_f0bk_places_baseline * 100;
...
%% ======================== f0bk_body ========================
body_LH = mean(f0bk_body(1:180,:),2) / avg_f0bk_body_baseline * 100;
body_RH = mean(f0bk_body(181:360,:),2) / avg_f0bk_body_baseline * 100;
...
%% ======================== f0bk_tools ========================
tools_LH = mean(f0bk_tools(1:180,:),2) / avg_f0bk_tools_baseline * 100;
tools_RH = mean(f0bk_tools(181:360,:),2) / avg_f0bk_tools_baseline * 100;
